...
const Dashboard: React.FC = () => {
  ...
  const loadDashboardStats = async () => {
    try {
      ...
      setStats({
        customers: Array.isArray(customersRes) ? customersRes.length : 0,
        menuItems: Array.isArray(menuItemsRes) ? menuItemsRes.length : 0,
        orders: Array.isArray(orders) ? orders.length : 0,
        staff: Array.isArray(staffRes) ? staffRes.length : 0,
        totalRevenue,
        recentActivity: Array.isArray(logsRes) ? logsRes.length : 0
      });
    } catch (error) {
